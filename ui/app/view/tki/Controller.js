Ext.define('koyoku.view.tki.Controller', {
	...
	simpan_pemeliharaan: function(status) {
		try {
			...
			if(!form.getForm().isValid()) {
				Ext.Msg.alert('Perhatian', "Periksa kembali form");
				return false;
			}
			...
			if (data_barang.length == 0) {
				Ext.Msg.alert('Perhatian', "Barang tidak boleh kosong");
				return false;
			}
	...
	telaah_pemeliharaan_sub_kegiatan: function() {
		var me = this,
			cmp = Ext.getCmp("page_renbut"),			
			grid = cmp.down("grid_pemeliharaan"),
			grid_form = cmp.down("#grid_form_pemeliharaan"),
	...
	simpan_telaah_pemeliharaan_sub_kegiatan: function() {
		try {
			var me = this,
				cmp = Ext.getCmp("page_renbut"),
				win = Ext.getCmp("form_telaah_pemeliharaan_sub_kegiatan"),				
				form = win.down("form"),
				grid = win.down("grid"),
				data_barang = [],
				params = {};				
