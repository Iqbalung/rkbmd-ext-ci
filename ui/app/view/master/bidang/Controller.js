Ext.define('koyoku.view.master.bidang.Controller', {
    extend: 'Ext.app.ViewController',
    requires: [

    ],
    alias: 'controller.bidang',
    add: function() {
        var cmp = Ext.getCmp("page_bidang"),
        ...
        if (parent.length > 0) {
            ...
            form = cmp.windowForm.down("form");
            form.down("[name=BIDANG_ID]").setValue(parent[0].get("BIDANG_ID"));
            cmp.windowForm.show(); 
        } else {
            form = cmp.windowForm.down("form");
            form.reset();
    ...
    save: function() {
        var me = this,
            cmp = Ext.getCmp("page_bidang"),
            grid = cmp.down("tree_bidang"),
            form = cmp.windowForm.down("form");
        if (form.isValid()) {
            form.submit({
                url:  api.siteurl + '/Bidang/save',
